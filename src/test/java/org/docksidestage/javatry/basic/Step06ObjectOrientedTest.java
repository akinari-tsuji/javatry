...
//        class SwimRobot{} extends Robot implements Swimmer {}
    ...
    public void test_objectOriented_writing_generalization_extractToAbstract() {
        // done jflute 次回1on1ここのふぉろー (2026/01/07)
        // your confirmation code here
        Dbms sql = new St6MySql();
        log(sql.doBuildPagingQuery(100, 10));
    ...
    public void test_objectOriented_writing_withPackageRefactoring() {
        ...
        // done jflute Contextを悪用して体力を減らす by akinari.tsuji (2026/02/04)
    ...
    public void test_objectOriented_zoo() {
        ...
        // _/_/_/_/_/_/_/_/_/_/
        // done jflute 次回1on1, Zombieのフォロー (2026/01/21)
        // #1on1: CatZombie, DogZombie, Good (2026/05/13)
        // 別に猫のゾンビがいてもいいだろう by つじさん
        // 
        // Human human = new Human("jflute");
        // human.acceptZombie(zombieKin);
        // ここから、この human インスタンスは Zombie 振る舞い
        // → 状態が変わるだけ？ by つじさん
        // 状態なのであれば、ベタの実装表現としてAnimalにisZombie持たせる
        //  e.g. protected boolean isZombie;
        // ぼくらが「あれゾンビだー」って指差しているのは厳密には「あれゾンビ状態になった動物だー」と言っている。
        // この場合、「ゾンビ == ゾンビ状態になった動物」と解釈できる。
        // もし、こういう解釈なら、既存のjavatryのZombieはおかしい実装である。
        //
        // 一方で、ゾンビ王国の王子様のお話、別のゾンビ王国のお姫様と結婚して、ゾンビの子供が生まれて幸せ...
        // という世界観だったら... (ゾンビは基本的に人型)
        // これだったら、既存のjavatryのZombieも悪くないかも。
        //
        // まあ結局、ゾンビという概念をどう捉えるか次第。
        //
        // _/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/
        // その業務概念の捉え方次第で、実装はがらりと変わる。
        // _/_/_/_/_/_/_/_/
        //
        // 業務概念を間違って捉えて実装すると、そのギャップが徐々に顕在化してきてつらくなる。
        // プログラムが大きくなればなるほどギャップが目立つようになってくる。
        // 逆に言うと、プログラムが小さい時代はギャップも小さいので、間違っててもどうにかなっちゃう。
        // これが危ない。どうにかなっちゃうから、直さずに進んでします。
        //
        // 業務概念を正しく捉えるって意外に難しいこと。
        // 業務理解ってそんな簡単なことじゃない。
        
        // #1on1: DDDの業務設計の話 (2026/05/13)
        // 業務設計のところに注目して勉強し直してみるといいかも。
        
        // #1on1: is-a, can-do, has-a の復習 (2026/05/13)
        
        // #1on1: 業務設計の思考が、実装手法に影響されるか？ (2026/05/13)
        // 業務設計のレイヤー次第。
        // o 要求定義 // 営業さんやプロダクトオーナーから？
        // o 要件定義 // ここは完全に業務的な話だけ
        // o 外部設計 // ここは画面とかもあるけどわりと業務で閉じた設計
        // o 内部設計 // ここはかなり実装を意識した設計
        // o 詳細設計 // ここはそーとー実装を意識した設計 (実装設計だよね)
        // o 実装(作業)
        // o テスト
        // 事業会社だと、↑の工程を一緒くたにやっていて、線引きがあまりない。
        
        // TODO jflute 次回もうちょい深掘り (2026/05/13)
        // (DB設計の論理設計と物理設計のお話も)
    }
}
