...
public class Step07ExceptionTest extends PlainTestCase {
   ...
   +--------------------+        +-------------------+
    ...
    public void test_exception_nullpointer_refactorCode() {
        try {
            ...
            // done jflute 次回1on1 (2026/02/18)
            // #1on1: 最新のJavaだと、nullの変数を教えてくれるようになった (2026/03/04)
            // (ようやっと)
            // statementと行の違い。statementとテキスト上の見栄えは切り離されている。
            // 他の言語の例。e.g. Ruby
            log(sum);
        } catch (NullPointerException e) {
            log(e);
    ...
    public void test_exception_checkedException_basic() {
        ...
        try {
            ...
            // （システム本来の処理とは別の経路で管理するために例外クラスを投げる？）
            // #1on1: ↑これはいわゆる業務例外を指してる。e.g. TicketSoldOutException by jflute
            // throwという特殊文法を使って、引数でもなく戻り値でもない経路で処理の中断を伝えていく。
            //
            // 想定外の例外はどこかで最終防御壁的なcatchがある、という感じでしょうか？
            // プロセスを止めないために、エラーを管理してあげるためのもので、想定されるエラーと想定外のエラーで止め方が異なる？
            // #1on1: Good, その通り。大抵のフレームワークや仕組みで「最終防御壁的なcatchがある」がある。
            // 仮にフレームワークがやらなくても、最終的にJava自体がcatchして、コンソールに例外情報を出す。
            // ただ、コンソールに出すだけだとぼくらは辛いので、ちゃんとフレームワークで処理して専用のログファイルに出力する。
            // オーソドックスには:
            // o 想定内の例外は、自分たちのどこかでcatch
            // o 想定外の例外は、仕組みの中の最終防御でcatch
            // (時々、アバウトにcatch(RuntimeException)とかして想定内も想定外もひっくるめて処理することはあるけど)
            //
    ...
    public void test_exception_cause_basic() {
        ...
        try {
            ...
            // #1on1: 例外チェーン (2026/03/04)
            // throwって1つしか投げられない、文法的に。
            // でも、取り扱いたい例外は複数ある。
            // 落ちたという一つのイベントに付き、複数の例外が発生している。
            // でも投げられるのは一つ。なので、例外に例外を紐づけて投げてるのは一つだけど、
            // チェーンでつながってるから、実質複数の例外を投げてるに等しい。
        }
    }
    ...
    public void test_exception_translation_debugChallenge() {
        try {
            ...
            // #1on1: 末端の例外throwする人は、全体像を知らないので、どれだけ頑張ってもデバッグ情報が足りない (2026/03/04)
            // だから、全体像をある程度知っている人、例えばハンドル会社がcatchして、ハンドル会社として知っている情報を載せて例外throwし直す。
            // そして車製造会社も同じようにcatchして...Dealerもcatchして...というようにそれぞれのレイヤー情報を積み上げて、
            // 全体像がわかる例外メッセージにする。(それぞれのレイヤーが持ってるデバッグ情報を全部かき集める)
            //
            // だから、例外というのは、一つの落ちたイベントにつき、複数の例外が必要になる。
            // なので、例外が例外を持つ例外チェーンの仕組みが必要になる。
            //
            /*
                 catch        catch        catch
     <---- 例外  /      例外  /     例外    /    例外
  ...
  /\           |     |        PK           ^^v
             ...
             */
            // カスタムの例外クラスを作る意義は？
            // o ピンポイントでcatchして業務的にリカバリ処理を入れやすくなる
            //   (現時点でcatchが不要でも、いつか必要になるかもしれないから、カスタムの例外の方が柔軟性は高い)
            //
